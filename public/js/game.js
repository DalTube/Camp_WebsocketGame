...
console.log('MAX_GAME_STAGE : ', MAX_GAME_STAGE);
...
function gameLoop(currentTime) {
  ...
  if (!gameover && !waitingToStart) {
    ...
    const isStageChange = score.update(
      deltaTime,
      stageAsset.data[currnetStage - 1],
      currnetStage < MAX_GAME_STAGE ? stageAsset.data[currnetStage] : null,
    );

    if (isStageChange && currnetStage < MAX_GAME_STAGE) {
      stage.update(stage.getStage() + 1);
    }
    // stage.update();
  }
